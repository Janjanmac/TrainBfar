...
onMounted(async () => {
  try {
    ...
    if(route.query.province) selectedProvince.value = route.query.province.toUpperCase()
    if(route.query.municipality) selectedMunicipality.value = route.query.municipality.toUpperCase()
  } catch (err) {
    console.error("Error loading DFW records:", err)
  }
})
...
const groupedData = computed(() => {
  ...
  filtered.forEach(r => {
    ...
    if (!grouped[prov][muni]) grouped[prov][muni] = []
    grouped[prov][muni].push(r)
  })

  // Sort provinces, municipalities, and last names
  const sorted = {}
  Object.keys(grouped).sort((a,b) => a.localeCompare(b)).forEach(prov => {
    const muni = grouped[prov]

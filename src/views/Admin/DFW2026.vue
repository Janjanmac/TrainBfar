  ...
  <div class="page-container">
    ...
    <div :class="['sidebar', { open: sidebarOpen }]">
      ...
      <ul>
        ...
        <li><router-link to="/DFWArchive2026">DFW Archive</router-link></li>
    ...
    <div :class="['page', { shifted: sidebarOpen }]">
      ...
      <div class="top-header">
        ...
        <h1 class="header-title">LIST OF DEPUTIZED FISH WARDEN (2026)</h1>
      </div>
...
onMounted(async () => {
  const snap = await getDocs(collection(db,"dfwRecords2026"))
...
async function saveRecord() {
  ...
  if(editIndex.value !== null){
    ...
    await setDoc(doc(db,"dfwRecords2026",existing.firebaseId), newRecord)
    records.value[editIndex.value] = {...newRecord, firebaseId: existing.firebaseId}
    editIndex.value = null
  } else {
    const docRef = await addDoc(collection(db,"dfwRecords2026"), newRecord)
...
async function archiveRecord(row){
  ...
  await addDoc(collection(db,"DFWArchive2026"), {...data, status:"ARCHIVED"})
  if(firebaseId) await deleteDoc(doc(db,"dfwRecords2026",firebaseId))

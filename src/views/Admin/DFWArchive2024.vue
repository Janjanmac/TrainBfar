<template>
  <div class="page">
    ...
    <div class="top-header">
      <button class="back-btn" @click="$router.push('/DFWARDENDashboard')">Back</button>
      <h1 class="header-title">ARCHIVED DEPUTIZED FISH WARDEN (2024)</h1>
    </div>
...
// LOAD ARCHIVE DATA
onMounted(async () => {
  const snap = await getDocs(collection(db,"DFWArchive2024"))
...
async function restore(row){
  try{
    ...
    await addDoc(collection(db,"dfwRecords2024"),{
      ...data,
      status:"ACTIVE"
    })
    ...
    // DELETE FROM ARCHIVE
    if(firebaseId){
      await deleteDoc(doc(db,"DFWArchive2024",firebaseId))
    }
...
async function removeForever(row){
  try{
    ...
    if(row.firebaseId){
      await deleteDoc(doc(db,"DFWArchive2024",row.firebaseId))
    }

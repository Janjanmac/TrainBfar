  ...
  <div class="page-container">
    ...
    <div :class="['sidebar', { open: sidebarOpen }]">
      ...
      <ul>
        ...
        <li><router-link to="/DFWArchive2025">DFW Archive</router-link></li>
...
async function saveRecord() {
  ...
  if(editIndex.value !== null){
    ...
    await setDoc(doc(db,"dfwRecords2025",existing.firebaseId), newRecord)
    records.value[editIndex.value] = {...newRecord, firebaseId: existing.firebaseId}
    editIndex.value = null
  } else {
    const docRef = await addDoc(collection(db,"dfwRecords2025"), newRecord)
...
async function archiveRecord(row){
  ...
  await addDoc(collection(db,"DFWArchive2025"), {...data, status:"ARCHIVED"})
  if(firebaseId) await deleteDoc(doc(db,"dfwRecords2025",firebaseId))
...
<style scoped>
/* Gamitin mo lang style ng 2024 version, pareho lang */
